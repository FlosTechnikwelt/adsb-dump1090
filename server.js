  ...
  for (const plane of aircraftList) {
    if (plane.hex) {
      ...
      db.run(
        ...
        [
          plane.hex, //Muss immer vorhanden sein
          plane.flight || null, //Flugnummer, falls vorhanden
          plane.alt_baro || null, //Barometrische Höhe, falls vorhanden
          plane.gs || null, //Ground Speed, falls vorhanden
          plane.track || null, //Kurs, falls vorhanden
          plane.lat || null, //Breitengrad, falls vorhanden
          plane.lon || null, //Längengrad, falls vorhanden
          plane.squawk || null, //Squawk Code, falls vorhanden
          aircraftType, //Flugzeugtyp, falls vorhanden
          manufacturer, //Hersteller, falls vorhanden
          photoUrl, //Foto URL, falls vorhanden
        ],
        function (err) {
          if (err) {
...
//Ruft Daten von der konfigurierten externen API ab. Keine lokale Fallback-Datei mehr.
//Die abgerufenen Daten werden in der Datenbank gespeichert und an den Client gesendet.
app.get("/api/aircraft", async (req, res) => {
  if (!config.apiUrl) {
    console.error(preserve, "No apiUrl configured in config.json. Aborting request.");
    return res
      .status(500)
      .send("No API URL configured. Set 'apiUrl' in config.json to enable this endpoint.");
  }
